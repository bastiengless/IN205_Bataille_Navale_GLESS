...
public class Game {
	...
	public Game init() {
		if (!loadSave()) {
			...
			Board opponentBoard = new Board("Opponent Board");

			// init this.player1 & this.player2
			List<AbstractShip> myShips = createDefaultShips();
			this.player1 = new Player(playerBoard, opponentBoard, myShips);

			List<AbstractShip> oppShips = createDefaultShips();
			this.player2 = new PlayerAI(opponentBoard, playerBoard, oppShips);
